...
const List = ({adsTitle,message,vote,id,fetchData,adsId}) =>{
    ...
    <div class="flex gap-5">
        <div className="flex gap-3 justify-center items-center">
            <div class="h-10 w-10 flex justify-center items-center"><North fontSize='large' onClick={()=>axios.post(`${BackendIP}/blacklist/vote`,{username,response:true,id}).then(res=>{
                if(res.data){
        ...
        <div className="flex gap-3 justify-center items-center">
            <div class="h-10 w-10 flex justify-center items-center"><SouthOutlined fontSize='large' onClick={()=>axios.post(`${BackendIP}/blacklist/vote`,{username,response:false,id}).then(res=>{
                if(res.data){
                    window.alert("Voting is successfull")
                }else{
                    window.alert("You are already Voted")
    ...
    return(
        <div className="fixed h-screen w-screen bg-black/30 -top-5 left-0 flex justify-center items-center">
            <div className=" w-96 bg-white border rounded-xl">
                ...
                <textarea className='h-40 w-full border outline-none p-3' onChange={e=>{setMessage(e.target.value)}}></textarea>
